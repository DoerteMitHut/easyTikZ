...
void findShapes(cv::Mat src, std::vector<std::vector<cv::Point2d>>& shapes,unsigned int maxVertCount = 6);
void findShapeCentroids(std::vector<std::vector<cv::Point>> shapes, std::vector<cv::Point2d>& centroids);
void connectorImage(cv::Mat srcShapes,cv::Mat srcBin, cv::Mat& dst,unsigned int dilationDepth = 4, int dilationShape = cv::MORPH_RECT, cv::Size kernelSize = cv::Size(9,9),cv::Point anchor = cv::Point(-1,-1));
